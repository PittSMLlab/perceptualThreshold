function [velL,velR]=profileGenSym(mainSpeeds,perturbationSizes,pertDuration,washDuration, reps)
...
for i=1:M
    for j=1:N
        for k=1:2 %L and R
            columnP=mainSpeeds(i)*ones(P+W,1) - [zeros(W,1); perturbationSizes(j)*ones(P,1)]/2;
            columnC=mainSpeeds(i)*ones(P+W,1) + [zeros(W,1); perturbationSizes(j)*ones(P,1)]/2;
            switch k
                case 1
